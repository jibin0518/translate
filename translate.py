...
def fix_text(text):
    ...
    text = text.replace("III", "I'm")
    text = text.replace("'m", "I'm")
    text = text.replace("Im", "I'm")
    text = text.replace("lm", "I'm")
...
def fix_eul_reul(text):
    def replace_match(match):
        full = match.group(0)   # 예: 버튼틀
        word = match.group(1)   # 예: 버튼

        if full in EXCEPTION_WORDS:
            return full

        last_char = word[-1]
        return word + ("을" if has_batchim(last_char) else "를")

    return re.sub(r'([가-힣]+)틀(?=\s|$|[.,!?])', replace_match, text)
    '''
def fix_tl_to_reul(text):
    # 한글 단어 + '틀'이 문장 끝 / 공백 / 구두점 앞에 있을 때
    return re.sub(r'([가-힣]+)틀(?=\s|$|[.,!?])', r'\1를', text)

def text_fix(text):
    replacements = {
        "어두위": "어두워","어무위" : "어두워","심치어": "심지어","엇조": "었죠","이특": "이득",
        "없조": "없죠","옆죠": "였죠","하조": "하죠","도료": "도로","뛰어터조" : "뛰어났죠","넷조" : "났죠",
        "뛰어낫조" : "뛰어났죠","잇없" : "있없","있있" : "있었","음악울" : "음악을","음악음" : "음악을","상처클" : "상처를",
        ...
        "걸어쩄으면" : "걸어줬으면","걸어짓으면" : "걸어줬으면","꺼쨌" : "꺼냈","말햇" : "말했", "햇권" : "했던", "햇던" : "했던",
          ...
          "감동올" : "감동을","너느" : "너는","알잡아" : "알잖아","것클" : "것을","없년" : "없던","시계름" : "시계를","햇듯이" : "했듯이",
        ...
        "켓군" : "겠군","켓균" : "겠군","기억블" : "기억을","걸없던" : "걸었던","느낌블" : "느낌을","컷지" : "렀지","균요" : "군요","잇나" : "있나","햇는" : "했는","부혹게" : "부를게",
        "활 수" : "할 수",
    }
    eng_replacements={
        "We\"l" : "We\"ll",";" : ",",
